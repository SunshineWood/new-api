        ...
        <>
          <article>
            ...
            <p>
              {cacheCreateTokens > 0 && (
                  <p>{i18next.t(
                      '缓存创建价格：${{price}} * {{cacheCreateRatio}} = ${{total}} / 1M tokens (缓存倍率: {{cacheCreateRatio}})',
                      {
                        price: inputRatioPrice,
                        total: inputRatioPrice * cacheCreateRatio,
            ...
            <p>
              {cacheCreateTokens > 0 ?
                  i18next.t(
                      '提示 {{input}} tokens/ 1M tokens * ${{price}} + 缓存创建 {{cacheCreate}} tokens * {{cacheCreateRatio}} / 1M tokens * {{createCachePrice}} + 补全 {{completion}} tokens / 1M tokens * ${{compPrice}} * 分组 {{ratio}} = ${{total}}',
                      {
                        input: inputTokens,
                        cacheCreate: cacheCreateTokens,
                        cacheCreateRatio: cacheCreateRatio,
                        price: inputRatioPrice,
                        createCachePrice: (inputRatioPrice * 1.25).toFixed(2),
            ...
            <p>
              {i18next.t(
                  '音频提示 {{input}} tokens / 1M tokens * ${{audioInputPrice}} + 音频补全 {{completion}} tokens / 1M tokens * ${{audioCompPrice}} = ${{total}}',
                  {
                    ...
                    audioCompPrice: audioRatio * audioCompletionRatio
                        * inputRatioPrice,
